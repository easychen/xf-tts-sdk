...
function text2sendjson( $text )
{
    ...
    $send_data = ['common'=>$common,'business'=>$business,'data'=>$data];return  json_encode( $send_data );
}

function get_xf_wss_info( $url = '/v2/tts' , $host = 'tts-api.xfyun.cn' )
{
    ...
    $final_url = 'wss://'.$host.$path;

    return ['url'=>$final_url,'path'=>$path,'host'=>$host];
}